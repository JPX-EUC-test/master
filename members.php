...
echo '<a href="./members.php' . '?base=' . date('Y-m-d', ($_GET['base'] ? strtotime($_GET['base']) : time()) - 86400) . '">前日</a>&nbsp;&nbsp;';

echo '<a href="./members.php' . '">最新</a>&nbsp;&nbsp;';

echo '<a href="./members.php' . '?base=' . date('Y-m-d', ($_GET['base'] ? strtotime($_GET['base']) : time()) + 86400) . '">翌日</a><br /><br />';

echo '<a href="./members_export.php' . ($_GET['base'] ? '?base=' . $_GET['base'] : NULL) . '">ダウンロード</a><br /><br />';
...
while($res = pg_fetch_object($que))
{
  ...
  echo $res->base;
  echo '</td><td>';
  echo $res->member_code;
  echo '</td><td>';
  echo $res->member_name;
  echo '</td><td>';
  echo $res->manager_code;
  echo '</td><td>';
  echo $res->manager_name;
  echo '</td></tr>';
}
echo '</table>';
